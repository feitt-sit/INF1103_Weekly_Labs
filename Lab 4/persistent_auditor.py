...
def load_inventory():
    try:
        with open(inventory_file, "r") as f:
            ...
            lines_history = content[1].strip() if len(content) > 1 else ""
            history = [int(x) for x in lines_history.split(",")] if lines_history else []
...
def save_inventory(total, history):
    with open(inventory_file, "w") as f:
        f.write(f"{total}\n")
        f.write(",".join(str(item) for item in history))
...
while True:
    ...
    history.append(validated_input)
